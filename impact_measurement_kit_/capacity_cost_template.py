...
def main():
    parser = argparse.ArgumentParser(
        description="Simple capacity and cost planning from measured inference metrics."
    )
    ...
    parser.add_argument(
        "--output-csv",
        default="impact_measurement_kit_/deployment_capacity_scenario.csv",
    )
    ...
    instances_needed = math.ceil(
        (sec_per_alert * peak_alerts_per_second) / max(args.utilization_target, 1e-9)
    )
    ...
    out = {
        ...
        "annual_electricity_cost_eur_if_peak_24_7": cost_electricity_per_alert
        * annual_alerts_at_peak_24_7
        if not math.isnan(cost_electricity_per_alert)
        else math.nan,
    ...
    if args.cloud_instance_hourly_eur is not None:
        ...
        out["annual_cloud_compute_eur_if_peak_24_7"] = (
            instances_needed * args.cloud_instance_hourly_eur * 24 * 365
        )
