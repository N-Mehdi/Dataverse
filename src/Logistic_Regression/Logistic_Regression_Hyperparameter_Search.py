...
def make_preprocessor(numeric_cols, categorical_cols):
    # Logistic Regression requires scaling — toujours activé
...
def parameter_grid():
    ...
    - C         : force de régularisation inverse (petit C = forte régularisation)
    - penalty   : type de régularisation (l1 nécessite solver liblinear/saga,
                  l2 compatible avec tous, elasticnet avec saga uniquement)
    - solver    : algorithme d'optimisation
    - max_iter  : nb max d'itérations pour la convergence
    - class_weight : gestion du déséquilibre de classes
    """
    combos = []

    # Pénalité L2 — compatible lbfgs et saga
    for C, solver, max_iter, class_weight in itertools.product(
        [0.01, 0.1, 1.0, 10.0],
        ["lbfgs", "saga"],
        [500, 1000],
        [None, "balanced"],
    ...
    # Pénalité L1 — uniquement avec liblinear ou saga
    for C, solver, max_iter, class_weight in itertools.product(
        [0.01, 0.1, 1.0, 10.0],
        ["liblinear", "saga"],
        [500, 1000],
...
def select_best_model(train_df, n_splits=5):
    ...
    print(f"\nRecherche hyperparamètres pour logistic regression...")
    ...
    best_specs = {"logistic": best_params}
...
def main():
    ...
    out_dir = sys.argv[2] if len(sys.argv) > 2 else "output/logistic_hyper"
