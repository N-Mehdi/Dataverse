...
def build_predictions_from_scores(
    silence_df: pd.DataFrame, scores: np.ndarray
) -> pd.DataFrame:
    pred_df = silence_df[["airport", "airport_alert_id", "decision_time"]].copy()
    ...
    pred_df = (
        ...
        .sort_values(["airport", "airport_alert_id", "prediction_date"])
...
def main():
    ...
    print(
        f"Nb alertes : {pred_df[['airport', 'airport_alert_id']].drop_duplicates().shape[0]}"
    )
