...
def compute_theta_results(
    ...
    alert_stats = []
    for (airport, alert_id), group in raw_df.groupby(
    ...
    pred_sorted = pred_df[
        ["airport", "airport_alert_id", "predicted_date_end_alert", "confidence"]
...
def main():
    raw_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "data/segment_alerts_all_airports_train.parquet"
    )
    pred_path = (
        sys.argv[2]
        if len(sys.argv) > 2
        else "output/model_comparison_with_logistic/test_predictions_long.csv"
    )
    out_dir = (
        Path(sys.argv[3])
        if len(sys.argv) > 3
        else Path("output/theta_analysis_logistic")
    )
